*============================================================================
*
*  PROJECT:     Economics of European Integration - Take-Home Exam 2025
*  COURSE:      20269 Economics of European Integration
*  AUTHORS:     Enrico Ancona, Simone Donoghue, Stefano Graziosi
*  DATE:        January 2025
*  
*  DESCRIPTION: This is the main analysis file for the take-home exam.
*               It addresses all 7 problems specified in the assignment,
*               covering topics including:
*               - Firm-level productivity analysis (Problems 1-4)
*               - Trade shock analysis - China shock (Problems 5-6)
*               - Political economy analysis using ESS data (Problem 7)
*
*  REQUIRED PACKAGES:
*               - outreg2:  ssc install outreg2, replace
*               - estout:   ssc install estout, replace
*               - levpet:   search levpet (for Levinsohn-Petrin estimation)
*               - prodest:  search prodest (for production function estimation)
*               - grstyle:  ssc install grstyle, replace (for graphs)
*               - coefplot: ssc install coefplot, replace (for graphs)
*
*  USAGE:       1. Set the working directory to the repository root
*               2. Run this file from start to finish
*               3. All outputs will be saved to:
*                  - Tables: output/tables/
*                  - Figures: output/figures/
*                  - Intermediate data: data/processed/
*
*  DIRECTORY STRUCTURE:
*               code/           - STATA .do files (this file is main_analysis.do)
*               data/raw/       - Original datasets
*               data/processed/ - Generated intermediate datasets
*               output/tables/  - LaTeX and HTML tables
*               output/figures/ - Graphs and maps
*               docs/           - Assignment PDF and references
*
*============================================================================
...
* NOTE: Update this section to match your local file structure
* The paths below assume you have cloned the repository
...
if ("`user'" == "stefanograziosi") {
	...
	global data "/Users/stefanograziosi/Documents/GitHub/20269-eei-report/data/raw"
	global processed "/Users/stefanograziosi/Documents/GitHub/20269-eei-report/data/processed"
	global tables "/Users/stefanograziosi/Documents/GitHub/20269-eei-report/output/tables"
	global figures "/Users/stefanograziosi/Documents/GitHub/20269-eei-report/output/figures"
}

if ("`user'" == "enricoancona") {
    global filepath "CHANGE_TO_YOUR_PATH"
	global output "$filepath/output"
	global data "$filepath/data/raw"
	global processed "$filepath/data/processed"
...
save "$processed/input_Q4", replace
...
clear all 
use "$processed/input_Q4", replace
...
save "$processed/part_I_cleaned_sample.dta", replace


** Plotting of the densities

use "$processed/part_I_cleaned_sample.dta", clear
...
clear all 
use "$processed/part_I_cleaned_sample.dta", replace
...
save "$processed/weights_pre.dta", replace
...
merge merge 1:m year country nace using "$processed/weights_pre.dta" "$processed/weights_pre.dta"
...
save "$processed/ChinaShock_by_region_year.dta", replace
...
merge merge 1:m year nace using "$processed/weights_pre.dta" "$processed/weights_pre.dta"
...
save "$processed/ChinaShock_by_region_year_us.dta", replace
...
use "$processed/ChinaShock_by_region_year.dta", clear
collapse (mean) sum_china_shock, by(nuts2 nuts2_name)
rename nuts2        	NUTS_ID
rename nuts2_name   	NAME_LATN
save "$processed/collapsedimpshock.dta", replace
 
	*—— Collapse instrument dataset ——————————————————————————————*

use "$processed/ChinaShock_by_region_year_us.dta", clear
collapse (mean) sum_china_shock_us, by(nuts2 nuts2_name)
rename nuts2        	NUTS_ID
rename nuts2_name   	NAME_LATN
save "$processed/collapsedimpshock_us.dta", replace
...
use "$processed/collapsedimpshock.dta", clear

merge merge 1:1 NUTS_ID using "$processed/collapsedimpshock "$processed/collapsedimpshock_us.dta"
drop _merge

save "$processed/sum_china_shock_merged", replace
...
shp2dta using "$data/shapefiles/NUTS_RG_20M_2013_3035.shp", database("$processed/nuts2_db.dta") coordinates("$processed/nuts2_coords.dta") genid(uid) replace

	*—— Load & merge region-cross-section shocks ——————————————————————————————*
use "$processed/nuts2_db.dta", clear
describe

merge 1:1 NUTS_ID using "$processed/sum_china_shock_merged.dta"
drop if _merge==1
drop _merge

	*—— Map average China shock ——————————————————————————————————————*
spmap sum_china_shock using "$processed/nuts2_coords.dta", id(uid) 						///
...
save "$processed/manuf_share.dta", replace

use "$processed/nuts2_db.dta", clear
describe

merge 1:1 NUTS_ID using "$processed/manuf_share.dta"
keep if _merge==3
drop _merge


spmap manuf_share using "$processed/nuts2_coords.dta", ///
    id(uid) ///
    fcolor(Greens)  ///
    clmethod(kmeans) ///
    clnumber(8) ///
...
use "$processed/sum_china_shock_merged.dta", clear
...
use "$processed/sum_china_shock_merged", clear
...
merge merge m:1 nuts2 using "$processed/sum_china_shock_merged "$processed/sum_china_shock_merged"
format nuts2 %-10s
drop if _merge==1
drop _merge
...
use "$data/ESS8e02_3", clear
	...
	save "$processed/ESS8_Italy_cleaned.dta", replace

merge merge m:1 nuts2 using "$processed/sum_china_shock_merged "$processed/sum_china_shock_merged.dta", keep(match)
...
merge merge m:1 nuts2 using "$processed/sum_china_shock_merged "$processed/sum_china_shock_merged", keep(match)
...
* panel A 
estout OLS_attitude IV_attitude using "$tables/EEI_Take_Home_VIIc.tex", replace style(tex) ///
keep(sum_china_shock) label cells(b(star fmt(3)) se(par fmt(3))) starlevels(* 0.10 ** 0.05 *** 0.01) ///
...
* panel B
estout First_Stage_IV_attitude using "$tables/EEI_Take_Home_VIIc.tex", append style(tex) ///
posthead("\noalign{\smallskip} \hline \noalign{\smallskip}" "\multicolumn{3}{l}{\emph{Panel B: First stage China Shock on China Shock US}} \\" "\noalign{\smallskip} \noalign{\smallskip}") ///
...
save "$processed/Q7cd.dta", replace
...
save "$processed/MP_cleaned.dta", replace
...
merge merge m:1 party_cmp using "$processed/MP_cleaned "$processed/MP_cleaned.dta", keep(match)

drop _merge

save "$processed/Q7ef.dta", replace
...
use "$processed/Q7ef.dta", replace
...
* panel A 
estout OLS_minority IV_minority using "$tables/EEI_Take_Home_VIIf.tex", replace style(tex) ///
keep(sum_china_shock) label cells(b(star fmt(3)) se(par fmt(3))) starlevels(* 0.10 ** 0.05 *** 0.01) ///
...
* panel B
estout First_Stage_IV_minority using "$tables/EEI_Take_Home_VIIf.tex", append style(tex) ///
posthead("\noalign{\smallskip} \hline \noalign{\smallskip}" "\multicolumn{3}{l}{\emph{Panel B: First stage China Shock on China Shock US}} \\" "\noalign{\smallskip} \noalign{\smallskip}") ///
keep(sum_china_shock_us) label cells(b(star fmt(3)) se(par fmt(3))) starlevels(* 0.10 ** 0.05 *** 0.01) mlabels(, none) collabels(, none) eqlabels(, none) stats(F_1st, fmt(a3 3) labels("Kleibergen-Paap F-statistic")) ///

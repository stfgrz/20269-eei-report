...
\setcounter{table}{0}
    ...
    \input{tables/Q6/EEI_Take_Home_VI_bb}

        Column 1 of Table \ref{tab:tfp_results} presents the results from running a simple regression of the post-crisis average TFP against the region-level China shock, controlling for 3-year lags of education, GDP and population. For the purpose of the analysis, country-year and industry-specific fixed effects were included. The main result is that the Import shock has a positive and statistically significant effect on average regional productivity.
    ...
    \subsection{Effects of Import Shock on Average Regional TFP (IV)}
        ...
        Column 2 of Table \ref{tab:tfp_results} reports the results of this 2SLS estimation. We can see that the estimated coefficient is now lower, and still significant. This indicates that the IV approach compensates for the previously hinted upward bias.
    ...
    \input{tables/Q6/EEI_Take_Home_VI_cc3}
...
%\renewcommand{\thetable}{7.\arabic{table}}
    ...
    \subsection{Linking Globalization to Support for Renewables Subsidies}
        ...
        \input{tables/Q7/EEI_Take_Home_VIIc}
    ...
    \subsection{Globalization and Cultural Backlash (VII.f \& VII.g)}

        \subsubsection{Impact of China Shock on Support for Minority Rights (VII.f)}

            In this final analysis, we examine whether exposure to the China shock also correlates with cultural attitudes, specifically the support for parties that advocate for underprivileged minority groups. We regress the \texttt{minority\_score} (a log-transformed measure of favorable references to minorities in the party's manifesto) against the regional China shock.

            \input{tables/Q7/EEI_Take_Home_VIIf}

            Table \ref{tab:results_minority} compares the OLS (Column 1) and IV (Column 2) estimates. The OLS coefficient is negative and significant, suggesting that regions more exposed to Chinese import competition tend to vote for parties with less favorable stances towards minorities.

            The 2SLS results (Column 2) confirm and strengthen this finding. The estimated coefficient is negative and statistically significant. Specifically, a one standard deviation increase in the China shock leads to a significant decrease in the \texttt{minority\_score} of the voted party. This implies a causal link: economic distress caused by globalization shifts voter support away from parties that champion minority rights and towards those with more exclusionary or nationalist platforms.

        \subsubsection{Economic vs. Cultural Determinants (VII.g)}

            The results from point (f) (minority rights) and point (c) (renewable subsidies) collectively paint a picture of the "globalization backlash." We find that the same economic shock—exposure to Chinese import competition—drives both:
            \begin{enumerate}
                \item \textbf{Policy Backlash}: Reduced support for public goods like renewable energy subsidies (as seen in VII.c).
                \item \textbf{Cultural Backlash}: Reduced support for minority rights (as seen in VII.f).
            \end{enumerate}

            This suggests that the economic grievances stemming from globalization (job losses, wage stagnation) are not just channeled into economic demands (like protectionism) but spill over into cultural and social domains. Voters in left-behind regions may adopt a "bundled" rejection of the liberal cosmopolitan agenda, perceiving environmentalism and minority rights as elite concerns that come at their expense. This aligns with the "cultural backlash" theory (Inglehart \& Norris, 2016), where economic insecurity seeks a scapegoat in "others" (minorities) and rejects the post-materialist values (environmentalism) of the winners of globalization. Thus, economic determinants (trade shocks) deeply influence cultural political outcomes.
